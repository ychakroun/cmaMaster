...
class GroupController extends Controller
{
    ...
    protected function ipCheck()
    {
        if (isset($_SERVER['HTTP_CLIENT_IP'])
            || !(in_array(@$_SERVER['REMOTE_ADDR'], array('127.0.0.1','90.78.116.136', 'fe80::1', '::1')) || php_sapi_name() === 'cli-server'))
        {
            $forwarde = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
            foreach ($forwarde as $key => $value) {
                if($forwarde[0]!=$value){
                    new RedirectResponse($this->generateUrl('home_page_homepage'));
                }
            }
        }
        return;
